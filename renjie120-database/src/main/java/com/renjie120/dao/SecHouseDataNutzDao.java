...
public class SecHouseDataNutzDao extends DaoTool<SecHouseData> { 
	@Override
	Chain makeUpdateValue(SecHouseData condition) {
		Chain ans = null;
		if (condition != null) {
	...
	@Override
	Cnd makeUpdateWhere(SecHouseData condition) {
		Cnd c = null;
		if (condition != null) {
	...
	@Override
	Cnd makeQueryCondition(SecHouseData condition) {
		Cnd c = null;
		if (condition != null) {
			if (condition.getId() != null && condition.getId() > 0) {
				c = (c == null ? Cnd.where("id", "=", condition.getId()) : c
